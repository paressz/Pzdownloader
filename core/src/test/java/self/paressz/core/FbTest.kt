...
class FbTest {
    ...
    fun fbDonwloadTest() {
        ...
        val service = Mockito.mock(RyzendesuFacebookService::class.java)
        ...
        val mCall = Mockito.mock(Call::class.java) as Call<RyzenDesuFbResponse>
        Mockito.`when`(service.downloadFacebookVideo(testUrl)).thenReturn(mCall)
        Mockito.`when`(mCall.enqueue(Mockito.any())).thenAnswer {
            val callback = it.getArgument<retrofit2.Callback<RyzenDesuFbResponse>>(0)
        ...
        mResult.enqueue(object : Callback<RyzenDesuFbResponse> {
            override fun onResponse(p0: Call<RyzenDesuFbResponse>, p1: Response<RyzenDesuFbResponse>) {
