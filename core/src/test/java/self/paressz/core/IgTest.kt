...
class IgTest {
    ...
    fun testIgDownload() {
        ...
        val call = Mockito.mock(Call::class.java) as Call<RyzenDesuIgResponse>
        ...
        Mockito.`when`(call.enqueue(Mockito.any())).thenAnswer { invocation ->
            val callback = invocation.getArgument<Callback<RyzenDesuIgResponse>>(0)
            callback.onResponse(call, Response.success(mockResponse))
        }
        val result = service.donwloadInstagramVideo(testUrl)
        result.enqueue(object : Callback<RyzenDesuIgResponse> {
